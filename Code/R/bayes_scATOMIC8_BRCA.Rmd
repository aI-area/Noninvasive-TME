...
```{python}
GBM_df = pd.read_csv('../../Data/scATOMIC_Macro/breast_expr_c88.csv')
...
```{python}
adata = ad.read_h5ad("I:/1+data/new/scATOMIC_reference.h5ad")
adata
```

```{python}
# select cell type as references
cell_list = ["Cancer Associated Fibroblasts","Fibroblasts","Breast Cancer","CD4+ T cell","CD8+ T cell","Endothelial Cells","Macrophage"]
...
```{python}
dc_adata.write_h5ad('../../Results/BRCA_bayes8_tpm.h5ad', compression='gzip')
```
